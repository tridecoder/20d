...
import { ChevronRight, ChevronLeft, Heart, Users, MessageCircle, AlertCircle, Check } from 'lucide-react';
...
export default function WeddingRSVPForm() {
  ...
  const canProceed = () => {
	...
	switch (step.id) {
	  ...
	  case 'companionName':
		return formData.companion === 'no' || formData.companionName.trim().length > 0;
	  default:
		return true;
	}
  };
  ...
  const shouldShowStep = (stepId) => {
	...
	if (stepId === 'companion' || stepId === 'companionName' || stepId === 'allergies' || stepId === 'diet' || stepId === 'song' || stepId === 'comments') {
			...
			{['yes', 'no'].map((option) => (
			  ...
			  >
				<div className="flex items-center gap-3">
				  {option === 'yes' ? <Heart className="text-rose-500" size={24} /> : <AlertCircle className="text-gray-400" size={24} />}
		...
		return (
		  <div className="space-y-4">
			{['yes', 'no'].map((option) => (
			  ...
			  >
				<div className="flex items-center gap-3">
				  <Users className={option === 'yes' ? 'text-rose-500' : 'text-gray-400'} size={24} />
				  <span className="text-xl">
					{option === 'yes' ? 'Sí, vendré acompañado/a' : 'No, iré solo/a'}
				  </span>
				</div>
			  </button>
			))}
  ...
  const currentVisibleStep = visibleSteps.findIndex(step => step.id === steps[currentStep].id);
	  ...
	  <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-8">
		...
		<div className="mb-8">
		  <div className="flex justify-between text-sm text-gray-500 mb-2">
			<span>Paso {currentVisibleStep + 1} de {visibleSteps.length}</span>
			<span>{Math.round(((currentVisibleStep + 1) / visibleSteps.length) * 100)}%</span>
		  </div>
		  <div className="w-full bg-gray-200 rounded-full h-2">
			<div 
			  className="bg-rose-500 h-2 rounded-full transition-all duration-300"
			  style={{ width: `${((currentVisibleStep + 1) / visibleSteps.length) * 100}%` }}
			/>
		  </div>
		</div>
		...
		{/* Navigation */}
		{steps[currentStep].id !== 'welcome' && steps[currentStep].id !== 'thanks' && (
